...
const EP_GITHUB_PR_BASE = 'https://github.com/osac-project/enhancement-proposals/pull/'
...
// The sourceRfe field has represented two related identifiers over time:
// older records contain the RFE's Jira key, while current OSAC records contain
// the enhancement-proposal pull request key (EP-n). PRD Review is selected by
// its issue key, so use the feature key for the latter schema.
const sourcePrdReviewKey = computed(() => {
  if (!sourceRfeKey.value) return null
  if (feature.value?.linkedRfeKey) return feature.value.linkedRfeKey
  return sourceRfeKey.value.startsWith('EP-') ? feature.value?.key : sourceRfeKey.value
})

const sourcePrdUrl = computed(() => {
  const latest = aiReview.value?.latest
  if (latest?.prdPrUrl) return latest.prdPrUrl
  if (/^EP-\d+$/i.test(sourceRfeKey.value || '')) {
    return EP_GITHUB_PR_BASE + sourceRfeKey.value.slice(3)
  }
  return null
})

const designPrUrl = computed(() => aiReview.value?.latest?.designPrUrl || null)

const hasDesignReview = computed(() => Boolean(aiReview.value?.latest && feature.value?.key))
            ...
            <div class="flex flex-wrap gap-3 mt-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span v-if="sourceRfeKey" class="flex items-center gap-1">
                ...
                <button
                  ...
                  :aria-label="`Open ${sourceRfeKey} in PRD Review`"
                  @click="crossNavigate('ai-impact', 'prd-review', { select: sourcePrdReviewKey })"
                >{{ sourceRfeKey }}</button>
                <a
                  v-if="sourcePrdUrl"
                  :href="sourcePrdUrl"
                  ...
                  title="Open PRD pull request on GitHub"
                  aria-label="Open PRD pull request on GitHub"
                >
              ...
              <span v-if="hasDesignReview" class="flex items-center gap-1">
                ...
                <button
                  ...
                  @click="crossNavigate('ai-impact', 'design-review', { select: feature.key })"
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                  title="Open Design pull request on GitHub"
                  aria-label="Open Design pull request on GitHub"
                >
                  <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
