...
| `project-profile` | `{ normalizeProjectProfile, createProjectProfileReader, createProjectProfileRegistry, createPublicationEnvelope, normalizeArtifactKey }` — validates published project identity/source metadata, reads profiles and immutable/current artifacts through storage, qualifies artifact keys under `projects/{projectId}/`, and provides publication envelopes with source, freshness, partial/error, and last-known-good fields. Production profile values come from the data repository; the in-memory registry is only a helper for isolated contract tests. |
